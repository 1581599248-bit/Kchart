...
def validate_shanghai(df: pd.DataFrame) -> None:
    ...
    cutoff_candidates = df.index[df["trade_date"] == pd.Timestamp("2026-07-24")].tolist()
    assert cutoff_candidates, "missing cutoff date"
    cutoff = cutoff_candidates[0] + 1
...
def main() -> None:
    ...
    print("live causal multi-index v15 audit OK")
