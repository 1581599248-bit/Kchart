...
from backend.app import indicators, reversal_patterns_v11, reversal_patterns_v14
...
def main() -> None:
    df = fetch_df()
    events = reversal_patterns_v14.find_index_reversals(df)
    ...
    assert not any(
        ...
        and abs(e["confirm_idx"] - best["confirm_idx"]) <= 1
        for e in events
    ), events
    assert len(events) <= reversal_patterns_v11.MAX_DISPLAY_EVENTS
    print("live reversal v14 validation OK")
