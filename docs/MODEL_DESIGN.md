...
模型代码实现接口见 `docs/ARCHITECTURE.md` 第 3 节 scoring.py。
（注1：回测功能已于 2026-08-03 下线，backtest.py 已删除；§5/§6 中回测执行与撮合相关内容保留作设计记录，恢复回测时再对照实现。）
（注2：TOP20 榜单页已于 2026-08-03 下线，scripts/bake_top20.py 已删除；打分模型本身保留，恢复榜单时按本节与 §6 口径重建烘焙链路。）
...
- 剔除：ST/*ST、停牌、上市不足 120 交易日、20 日中位成交额 < 5000 万（曾由 scripts/bake_top20.py 在榜单日期当日判定：`days_since_listing>=120` 且 `median_amount_cny_20>=5e7`；榜单页已下线，口径保留在此）。
