...
定稿日期：2026-08-01
本文档是 scoring.py / backtest.py 的设计依据，记录每个因子存在或被淘汰的理由及文献来源。
模型代码实现接口见 `docs/ARCHITECTURE.md` 第 3 节 scoring.py / backtest.py。
...
- 剔除：ST/*ST、停牌、上市不足 120 交易日、20 日中位成交额 < 5000 万（strict 视图已含大部分，成交额阈值在其上叠加）。
- 执行：信号日 t 收盘打分 → t+1 开盘价成交；一字涨停买不进 → 顺延排名下一位；一字跌停卖不出 → 持有至下一可交易日（`security_trading_status.hit_limit_up/down`）。
