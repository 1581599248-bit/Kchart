...
- 磁盘缓存 `data/api_cache/`（JSON，git 忽略）：stock_basic/trade_cal 12 小时整包缓存；K 线类（daily/adj_factor/index_daily）按 ts_code 单文件存原始数据，缓存最大日期 < 最新交易日时增量拉取（start_date=最大日+1）合并去重写回。
